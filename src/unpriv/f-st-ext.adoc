...
[[norm:fcsr_op_sz_acc]]
The floating-point control and status register, csr:fcsr[], is a RISC-V
...
csr:frm[] (csr:fcsr[] bits 7--5) and copies it into the least-significant three bits of
...
Accrued Exception Flags field csr:fflags[] (csr:fcsr[] bits 4--0).

[[norm:fcsr_rsv]]
Bits 31--8 of the csr:fcsr[] are reserved for other standard extensions. If
